...
if (!isNil "_tempReloadWeapon" &&  {((_weapon) == _tempReloadWeapon)}) then
{
	...
	private _cwm = _unit getVariable ["NIArms_altReloads_lastfireMode",""];
	private _isFLOn = _unit getVariable ["NIArms_altReloads_lastFlashlightmode",""];
	private _isIROn = _unit getVariable ["NIArms_altReloads_lastIRmode",""];
	private _zeroing = _unit getVariable ["NIArms_altReloads_lastZero",""];
	private _previousMuzzle = _unit getVariable ["NIArms_altReloads_lastMuzzle",""];
	//systemChat format ["Post-reload Zeroing value:%1",_zeroing];
	[_unit, _previousWeapon, _cwm] call CBA_fnc_selectWeapon;
	_unit setWeaponZeroing [_previousWeapon, _previousMuzzle, _zeroing];
