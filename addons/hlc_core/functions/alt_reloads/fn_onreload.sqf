...
private _isZeroed = _unit currentZeroing [_weapon,_muzzle] params ["_distance", "_zeroingIndex"];

//systemChat format ["Zeroing value:%1",_zeroingIndex];
_unit setVariable ["NIArms_altReloads_lastfireMode",_fireMode];
_unit setVariable ["NIArms_altReloads_lastFlashlightmode",_isFLOn];
_unit setVariable ["NIArms_altReloads_lastIRmode",_isIROn];
_unit setVariable ["NIArms_altReloads_lastZero",_zeroingIndex];
_unit setVariable ["NIArms_altReloads_lastMuzzle",_Muzzle];
